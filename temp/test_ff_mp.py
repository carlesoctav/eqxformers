...
def main(data_dir: str, report_every: int, num_workers: int, buffer_size: int) -> None:
    ...
    ds = load_from_disk(resolved_dir)
    ds = grain.MapDataset.source(ds).to_iter_dataset()
    
    
    
    # Use mp_prefetch for multiprocessing
    mp_options = grain.MultiprocessingOptions(
        num_workers=num_workers,
        per_worker_buffer_size=buffer_size,
    )
    mp_ds = ds.mp_prefetch(mp_options)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Iterate a dataset with multiprocessing and measure throughput.")
    parser.add_argument("--data-dir", default="~/data/fineweb", help="Path to the dataset directory (supports ~ expansion).")
    parser.add_argument(
        "--report-every",
        type=int,
        default=10_000,
