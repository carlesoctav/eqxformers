...
def main(args):
    ...
    ds = WithOptionsIterDataset(
        ds, DatasetOptions(min_shm_size=args.min_shm_size)
    )

    mp_options = grain.MultiprocessingOptions(
        num_workers=args.num_workers,
        per_worker_buffer_size=args.buffer_size,
    )
    mp_ds = ds.mp_prefetch(mp_options, sequential_slice=not args.strided)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Streaming MLM preprocessing test with mp_prefetch.")
    ...
    parser.add_argument("--num-workers", type=int, default=4, help="Number of mp workers.")
    parser.add_argument("--buffer-size", type=int, default=256, help="Per-worker buffer size.")
    parser.add_argument("--min-shm-size", type=int, default=1 << 20, help="Minimum array size to place in shared memory.")
    parser.add_argument("--strided", action="store_true", help="Use strided slicing instead of contiguous (sequential) slicing.")
